---
name: download-douyin-audio
description: Download the highest available source-quality audio from one Douyin link only when the user explicitly asks to 提取音频, 下载音频, extract audio, or download audio and the link belongs to douyin.com or iesdouyin.com. Do not activate for a bare Douyin link that the user only wants opened, read, summarized, or discussed.
---

# Download Douyin Audio

Use this Skill's wrapper for the shared deterministic downloader. Accept exactly one published-video URL whose normalized hostname is `douyin.com` or `iesdouyin.com`, or one of their subdomains.
...
- Run this Skill's `scripts/run.py`; the wrapper applies the fixed Douyin platform guard:
...
- The wrapper keeps `--expected-platform Douyin` fixed. Reject a mismatched platform, multiple URLs, a playlist-only URL, or an active livestream.
- Preserve the best source audio codec exposed by Douyin and yt-dlp. A container-only remux is acceptable; an unexpected codec change is a failure.
