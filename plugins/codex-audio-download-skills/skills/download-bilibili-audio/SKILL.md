...
description: Download the highest available source-quality audio from one Bilibili link only when the user explicitly asks to 提取音频, 下载音频, extract audio, or download audio and the link belongs to bilibili.com or b23.tv. Do not activate for a bare Bilibili link that the user only wants opened, read, summarized, or discussed.
---

# Download Bilibili Audio

Use this Skill's wrapper for the shared deterministic downloader. Accept exactly one published-video URL whose normalized hostname is `bilibili.com` or `b23.tv`, or one of their subdomains.
...
- Run this Skill's `scripts/run.py`; the wrapper applies the fixed Bilibili platform guard:
...
- The wrapper keeps `--expected-platform Bilibili` fixed. Reject a mismatched platform, multiple URLs, a playlist-only URL, or an active livestream.
- Preserve the best source audio codec exposed by Bilibili and yt-dlp. A container-only remux is acceptable; an unexpected codec change is a failure.
